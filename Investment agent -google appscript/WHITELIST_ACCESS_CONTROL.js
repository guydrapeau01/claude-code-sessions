...
var ADMIN_ID = '33834830566132027';        // You - full access to all commands

var DCF_ONLY_USERS = [                     // Friends - can ONLY use DCF command
  // Add your friends' Facebook IDs here (one per line)
  // Example: '1234567890',
  // To get a friend's ID: have them message your page, then check logs
];

var BLOCKED_USERS = [                      // Blocked users - no access
  // Add IDs here to block specific users
];
 ...
 */
function executeCommand_(parsed, userId) {
  // Access Control Check
  userId = String(userId || '').trim();
  
  // Check if user is blocked
  if (BLOCKED_USERS.indexOf(userId) >= 0) {
    return {
      success: false,
  ...
  // Check if user is DCF-only
  var isDCFOnly = (DCF_ONLY_USERS.indexOf(userId) >= 0);
  
  // If not admin and not in DCF-only list, deny access
  if (!isAdmin && !isDCFOnly) {
    return {
      success: false,
      message: '❌ You don\'t have access to this bot.\n\nContact the bot owner for access.'
    };
  }
  
  // DCF-only users can only use DCF and HELP commands
  if (isDCFOnly && parsed.command !== 'DCF' && parsed.command !== 'HELP') {
    return {
      success: false,
      message: '❌ You only have access to DCF command.\n\nTry: DCF <TICKER>\nExample: DCF AAPL'
    };
  }
...
function CheckMessengerMessages() {
  ...
  try {
    ...
    for (var i = 0; i < messages.length; i++) {
      ...
      Logger.log('Processing message from user: ' + senderId);
      ...
      // Parse and execute command (with userId for access control)
